...
while query < total_Query_Images:
    ...
    for seq1 in range(len(seq_best_match[0])):
        for seq2 in range(len(seq_best_match[1])):
            if seq_best_match[0][seq1] >= seq_best_match[1][seq2] or seq_top_k[0][seq1] == seq_top_k[1][seq2]:
                for seq3 in range(len(seq_best_match[2])):
                    if seq_best_match[1][seq2] >= seq_best_match[2][seq3] or seq_top_k[1][seq2] == seq_top_k[2][seq3]:
                        for seq4 in range(len(seq_best_match[3])):
                            if seq_best_match[2][seq3] >= seq_best_match[3][seq4] or seq_top_k[2][seq3] == seq_top_k[3][seq4]:
                                for seq5 in range(len(seq_best_match[4])):
                                    if seq_best_match[3][seq4] >= seq_best_match[4][seq5] or seq_top_k[3][seq4] == seq_top_k[4][seq5]:
                                        # print("seg-set index", top_k_idx[seq1],top_k_idx[seq2],top_k_idx[seq3])
                                        if seq_top_k[0][seq1] - seq_top_k[1][seq2] <= 2 and seq_top_k[1][seq2] - seq_top_k[2][seq3] <= 2 and seq_top_k[2][seq3] - seq_top_k[3][seq4] <= 2 and seq_top_k[3][seq4] - seq_top_k[4][seq5] <= 2:
                                            # print("seq index 3:", seq_best_match[2][seq3])
                                            loop_id += 1
                                            # print("id",loop_id)
                                            seq_tmp = []
                                            sequence_score.append(seq_best[0][seq1]+seq_best[1][seq2]+seq_best[2][seq3]+seq_best[3][seq4]+seq_best[4][seq5])
