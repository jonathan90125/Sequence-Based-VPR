...
ET=0.2# Entropy threshold, vary between 0-1.
ST=0.03 # similarity threshold
SN= 10 # bsets number
PR_1=0.00065# precision recall threshold
PR_2=0.008# precision recall threshold
...
query = 100 # start index for query
...
query_directory='./dataset/query/'       #Please modify according to your needs
ref_directory='./dataset/ref/'     #Please modify according to your needs
...
if PR_2 == 0.008:
    ...
    while query < total_Query_Images:
        ...
        while query_flag == 0 and fore_query > 0:
            ...
            score = conv_match_dotproduct(vector_4.transpose(), vector_5, regional_goodness_3,
                                          total_no_of_regions)
            # print("score2", score)
            fore_query -= 1
            if (score > pre_score) or (score < init_score - ST):
        ...
        # print("query seq:",tmp[0],tmp[1],tmp[2])
        ...
        for seq1 in range(len(seq_best_match[0])):
            # print("seq index 1:",seq_best_match[0][seq1] )
            for seq2 in range(len(seq_best_match[1])):
                if seq_best_match[0][seq1] > seq_best_match[1][seq2]:
                    # print("seq index 2:", seq_best_match[1][seq2])
                    for seq3 in range(len(seq_best_match[2])):
                        # print("seq index 3-1:", seq_best_match[2][seq3])
                        if seq_best_match[1][seq2] > seq_best_match[2][seq3]:
                            # print("seg-set index", top_k_idx[seq1],top_k_idx[seq2],top_k_idx[seq3])
                            if seq_top_k[0][seq1] - seq_top_k[1][seq2] <= 3 and seq_top_k[1][seq2] - seq_top_k[2][seq3] <= 3:
                                # print("seq index 3:", seq_best_match[2][seq3])
                                loop_id += 1
                                # print("id",loop_id)
                                seq_tmp = []
                                sequence_score.append(seq_best[0][seq1]+seq_best[1][seq2]+seq_best[2][seq3])
        ...
        if len(sequence_score):
            ...
            for k in range(len(final_results)):
                if abs((final_results[k][0] + final_results[k][2])/2 - (curr_seq[0] + curr_seq[2])/2) < 5 :
                    TP1 += 1
            pre_tp2 = TP2
            for k in range(len(final_results)):
                if abs((final_results[k][0] + final_results[k][2])/2 - (curr_seq[0] + curr_seq[2])/2) < 5:
        ...
        # print("sequence_results",sequence_results)

    precision = TP1/retrieved_num
    recall = TP2/ (query-100)
