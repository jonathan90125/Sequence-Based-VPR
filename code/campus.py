...
query_directory = './dataset/dataset_1/query/'  # Please modify according to your needs
ref_directory = './dataset/dataset_1/ref/'  # Please modify according to your needs
...
while ref < 101:
    ...
    fore_limit_1.append(tmp[1])
    fore_limit.append(tmp[2])
    ref+=1

np.savetxt("fore_limit_1_1.csv", fore_limit_1, delimiter=",")
np.savetxt("fore_limit_1.csv", fore_limit, delimiter=",")

fore_limit = np.loadtxt('fore_limit_1.csv', dtype=np.float, delimiter=',')
fore_limit_1 = np.loadtxt('fore_limit_1_1.csv', dtype=np.float, delimiter=',')
...
while query < total_Query_Images:
                            ...
                            if seq_best_match[2][seq3] > seq_best_match[3][seq4] and seq_best_match[3][seq4] > fore_limit[seq_best_match[2][seq3]]:
                                for seq5 in range(len(seq_best_match[4])):
                                    if seq_best_match[3][seq4] > seq_best_match[4][seq5] and seq_best_match[4][seq5] > fore_limit[seq_best_match[3][seq4]]:
                                        ...
                                        seq_tmp = []
                                        current_score = seq_best[0][seq1] + seq_best[1][seq2] + seq_best[2][seq3] + seq_best[3][seq4] + seq_best[4][seq5]
                                        # if abs(seq_best_match[2][seq3] - fore_limit[seq_best_match[0][seq1]]) < 5 and abs(seq_best_match[3][seq4]-fore_limit[seq_best_match[1][seq2]]) < 5 and abs(seq_best_match[4][seq5]-fore_limit[seq_best_match[2][seq3]]) < 5:
                                        #     current_score = current_score *1.2
                                        # if abs(seq_best_match[1][seq2] - fore_limit_1[seq_best_match[0][seq1]]) < 5 and abs(seq_best_match[2][seq3] - fore_limit_1[seq_best_match[1][seq2]]) < 5 and abs(seq_best_match[3][seq4] - fore_limit_1[seq_best_match[2][seq3]]) < 5 and abs(seq_best_match[4][seq5] - fore_limit_1[seq_best_match[3][seq4]]) < 5:
                                        #     current_score = current_score *1.2

                                        sequence_score.append(current_score)
    ...
    if len(sequence_score):
        ...
        for k in range(len(final_results)):
            l1 = final_results[k][0] - final_results[k][4]
            l2 = curr_seq[0] - curr_seq[4]
            # print(final_results[k][0], curr_seq[0])
            # print(final_results[k][5], curr_seq[5])
            xmax = min(final_results[k][0], curr_seq[0])
            xmin = max(final_results[k][4], curr_seq[4])
            # print(xmax)
            # print(xmin)
            overlap = max(0, xmax - xmin)
            iou = overlap / (l1 + l2 - overlap)
            print(iou)
            if iou > 0.5:
                TP1 += 1
        pre_tp2 = TP2
