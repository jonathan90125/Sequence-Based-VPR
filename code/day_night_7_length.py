...
while query < total_Query_Images:
    ...
    fore_query = fore_query
    while query_flag == 0 and fore_query > 0:
        vector_9, regional_goodness_9, no_of_good_regions_9 = read_query_images(fore_query)
        score = conv_match_dotproduct(vector_8.transpose(), vector_9, regional_goodness_8,
                                      total_no_of_regions)
        # print("score2", score)
        fore_query -= 1
        if (score > pre_score) or (score < init_score - ST):
            query_sequence_list.append(vector_9)
            tmp.append(fore_query + 1)
            query_flag = 1
        else:
            pre_score = score
    ...
    print("query seq:", tmp[0], tmp[1], tmp[2], tmp[3],tmp[4],tmp[5],tmp[6])
                                    ...
                                    if seq_best_match[3][seq4] > seq_best_match[4][seq5] and seq_best_match[4][seq5] > fore_limit[seq_best_match[3][seq4]]:
                                        for seq6 in range(len(seq_best_match[5])):
                                            if seq_best_match[4][seq5] > seq_best_match[5][seq6] and seq_best_match[5][seq6] > fore_limit[seq_best_match[4][seq5]]:
                                                for seq7 in range(len(seq_best_match[6])):
                                                    if seq_best_match[5][seq6] > seq_best_match[6][seq7] and seq_best_match[6][seq7] > fore_limit[seq_best_match[5][seq6]]:
                                                            seq_tmp = []
                                                            current_score = seq_best[0][seq1] + seq_best[1][seq2] + seq_best[2][seq3] + seq_best[3][seq4] + seq_best[4][seq5] + seq_best[5][seq6] + seq_best[5][seq7]
                                                            ...
                                                            seq_tmp.append(seq_best_match[6][seq7])
    ...
    if len(sequence_score):
        ...
        for k in range(len(final_results)):
            if abs((final_results[k][0] + final_results[k][6])/2 - (curr_seq[0] + curr_seq[6])/2) < 3:
                TP1 += 1
        pre_tp2 = TP2
        for k in range(len(final_results)):
            if abs((final_results[k][0] + final_results[k][6])/2 - (curr_seq[0] + curr_seq[6])/2) < 3:
